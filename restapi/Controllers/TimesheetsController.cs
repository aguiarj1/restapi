...
namespace restapi.Controllers
{
    ...
    public class TimesheetsController : Controller
    {
        ...
        [Produces(ContentTypes.Transition)]
        [ProducesResponseType(typeof(Transition), 200)]
        [ProducesResponseType(typeof(InvalidStateError), 404)]
        [ProducesResponseType(typeof(InvalidStateError), 409)]
        [ProducesResponseType(typeof(EmptyTimecardError), 409)]
        public IActionResult Approve(string id, [FromBody] Approval approval)
        {
            ...
            if (timecard != null)
            {
                ...
                //if timecard approver is timecard resource return error
                if (timecard.Resource == approval.Resource)
                {
                    return StatusCode(404, new InvalidStateError() { });
                }
        ...
        [ProducesResponseType(typeof(InvalidStateError), 409)]
        [ProducesResponseType(typeof(EmptyTimecardError), 409)]
        public IActionResult Correct(string id, [FromBody] Approval approval)
        {
            Timecard timecard = Database.Find(id);
            ...
            if (timecard != null)
            {
                ...
                //if timecard returner is timecard resource return error
                if (timecard.Resource == approval.Resource)
                {
                    return StatusCode(404, new InvalidStateError() { });
                }

                //return it by putting it in the draft stage
                var transition = new Transition(approval, TimecardStatus.Draft);
                timecard.Transitions.Add(transition);
                return Ok(transition);
            }
            else
